    ...
    if len(source_obj)>0:
        ...
        for obj in source_obj:
            ...
            if format == 'txt':
                ...
                log_file = open('../destination/log.txt','a+')
                subprocess.call('python '+name+'.'+format,stdout = log_file,stderr=log_file)
                print(name+'.'+format+' executed sucessfully, outputs are saved in destination/log.txt file')
                log_file.close()
            else:
